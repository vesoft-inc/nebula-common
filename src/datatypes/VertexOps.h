...
_loop:
    ...
    if (proto->kUsesFieldNames()) {
        detail::TccStructTraits<nebula::Tag>::translateFieldName(
...
template <>
template <class Protocol>
uint32_t Cpp2Ops<nebula::Tag>::serializedSize(Protocol const* proto, nebula::Tag const* obj) {
    uint32_t xfer = 0;
...
template <>
template <class Protocol>
uint32_t Cpp2Ops<nebula::Tag>::serializedSizeZC(Protocol const* proto, nebula::Tag const* obj) {
    uint32_t xfer = 0;
    xfer += proto->serializedStructSize("Tag");
...
_readField_vid : { proto->readBinary(obj->vid); }
...
_loop:
    ...
    if (proto->kUsesFieldNames()) {
        detail::TccStructTraits<nebula::Vertex>::translateFieldName(
            readState.fieldName(), readState.fieldId, readState.fieldType);
    }
...
template <>
template <class Protocol>
uint32_t Cpp2Ops<nebula::Vertex>::serializedSize(Protocol const* proto, nebula::Vertex const* obj) {
    uint32_t xfer = 0;
