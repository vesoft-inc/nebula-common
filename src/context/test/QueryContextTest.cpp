...
TEST(QueryContext, HistoryTest) {
    ...
    ASSERT_EQ(4, ctx.numValueVersions("v1"));
    const auto& hist1 = ctx.getValueHist("v1");
    auto it = hist1.begin();
    ...
    ctx.truncValueHist("v1", 5);
    ASSERT_EQ(4, ctx.numValueVersions("v1"));

    ctx.truncValueHist("v1", 2);
    ASSERT_EQ(2, ctx.numValueVersions("v1"));

    const auto& hist2 = ctx.getValueHist("v1");
